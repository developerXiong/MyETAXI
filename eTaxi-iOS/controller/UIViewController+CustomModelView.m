...
-(void)addNavigationBar:(NSString *)title
{
    ...
    [self addNavigationBar:COLORWITHRGB(0, 91, 201, 1) image:nil title:title action:@selector(dismissViewC)];
    
}

-(void)addNavigationBar:(NSString *)title LeftButtonAction:(SEL)action
{
    [self addNavigationBar:COLORWITHRGB(0, 91, 201, 1) image:nil title:title action:action];
}

-(void)addCleaerNavigationBar:(NSString *)title
{
    
    [self addNavigationBar:[UIColor clearColor] image:nil title:title action:@selector(dismissViewC)];
}

-(void)addNavigationBar:(UIColor *)color image:(UIImage *)image title:(NSString *)title action:(SEL)action
{
    /**
     *  添加整体的navigationBar
     */
...
-(void)addRightBtnWithTitle:(NSString *)title font:(CGFloat)font action:(SEL)action
{
    ...
    [rightBtn setFrame:CGRectMake(JDScreenS.width-100, 23, 120, 44)];
...
{
    [self addRightBtnWithTitle:title font:14 action:action];
}
