...
- (void)viewDidLoad {
    ...
    _sections = @[@"营收统计",@"违章",@"召车接单",@"我的钱包"];
    _rows = @[@[@"语音开关",@"每日发送时间"],@[@"语音开关"],@[@"语音开关",@"浮窗开关"],@[@"设置支付密码"]];
    _keys = [[JDShareInstance shareInstance] settingKeys];
    _classes = @[@[@"JDSettingSwitchCell",@"JDSettingTimeCell"],@[@"JDSettingSwitchCell"],@[@"JDSettingSwitchCell",@"JDSettingSwitchCell"],@[@"JDSettingMoneybagCell"]];
...
{
    NSString *key = [_keys[indexPath.section][indexPath.row] description]; // 每一行对应的key值，存储到系统偏好设置的时候对应的key
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0&&indexPath.row==1) {
        ...
        JDInputPassFirstViewController *IPVC = [[JDInputPassFirstViewController alloc] init];
