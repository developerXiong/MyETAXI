...
- (void)getFindNewPasswordWithPhoneNo:(NSString *)phone WithNewPassword:(NSString *)password validCode:(NSString *)validCode WithCompletion:(void (^)(NSString *, NSString *))block
{
...
#pragma mark-更换手机号码
- (void)getChangeNewPhoneWithPhoneNo:(NSString *)phone WithNewPhoneNo:(NSString *)nPhone WithType:(NSString *)type validCode:(NSString *)validCode WithCompletionBlock:(void (^)(NSString *, NSString *))block
{
    NSDictionary * inDic =[NSDictionary dictionaryWithObjectsAndKeys:phone,@"phoneNo",nPhone,@"newPhoneNo",PASSWORD,@"password",type,@"type",LOGINTIME,@"loginTime",validCode,@"validCode", nil];
    AFHTTPRequestOperationManager * manager =[AFHTTPRequestOperationManager manager];
    manager.responseSerializer=[AFJSONResponseSerializer serializer];
    manager.responseSerializer.acceptableContentTypes=[NSSet setWithObjects:@"text/html",@"application/json", nil];
    manager.requestSerializer.timeoutInterval=10;
...
{
    NSString *uuidOrderNo = [GetData getUniqueStrByUUID]; // 获取订单号

    NSDictionary * inDic =[NSDictionary dictionaryWithObjectsAndKeys:PHONENO,@"phoneNo",PASSWORD,@"password",LOGINTIME,@"loginTime",@"苏AJ08G9",@"licenseNo",@"005362",@"engineNo",point,@"cost",violation,@"violations",uuidOrderNo,@"orderNo",@"0",@"app", nil];
