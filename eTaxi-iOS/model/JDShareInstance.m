...
-(NSString *)settingKey:(JDKey)settingKey
{
    ...
    switch (settingKey) {
        ...
        case 5:
        {
            key = [keys[kMyMoneyBagIndexPath.section][kMyMoneyBagIndexPath.row] description];
        }
            break;
...
-(NSArray *)settingKeys
{
    NSArray *keys = @[@[@"incomeNotice",@"incomeTime"],@[@"trafficNotice"],@[@"callcarNotice",@"callcarFloating"],@[@"myMoneyBag"]];
